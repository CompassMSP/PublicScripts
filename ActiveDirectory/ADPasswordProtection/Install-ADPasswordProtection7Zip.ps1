Function Install-ADPasswordProtection {
    ...
    $StoreFilesInDBFormatFile = $StoreFilesInDBFormatLink -replace '[a-zA-Z]+://([a-zA-Z]+(\.[a-zA-Z]+)+)/[a-zA-Z]+/'
    $StoreFilesInDBFormatFileTXT = $($StoreFilesInDBFormatFile -replace '\.7z') + '.txt'
    ...
    } else { Write-Output '7Zip4Powershell Modules is already install.' }
    ...
    if ((Get-PSDrive C).free -gt 20GB) {
        ...
        if (-not $HIBPDBFilesExist) {
            ...
            try {
                Expand-7Zip -ArchiveFileName $StoreFilesInDBFormatFile -TargetPath 'C:\temp'
            }
            catch {
        ...
        Import-CompromisedPasswordHashes -Filename $StoreFilesInDBFormatFileTXT
